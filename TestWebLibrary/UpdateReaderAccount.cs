...
{
    internal class UpdateReaderAccount : test
    {
        [Test]
        ...
        public void testUpdateCategory(string username, string password)
        {
            ...
            for (int i = 7; i <= 11; i++)
            {
                ...
                {
                    string actual = "Hệ thống cập nhật tài khoản người đọc thành công và trả về trang Index";
                ...
                {
                    string actual = "Hệ thống báo lỗi không đủ dữ liệu để cập nhật tài khoản người đọc";
                    worksheet.Cell(i, 4).Value = actual;
                    if (CompareExpectedAndActual(expected, actual)) worksheet.Cell(i, 5).Value = "Passed";
                    else worksheet.Cell(i, 5).Value = "Failed";
                }
                ...
                {
                    string actual = "Hệ thống báo lỗi sai dữ liệu để cập nhật tài khoản người đọc mới";
                    worksheet.Cell(i, 4).Value = actual;
                    if (CompareExpectedAndActual(expected, actual)) worksheet.Cell(i, 5).Value = "Passed";
                    else worksheet.Cell(i, 5).Value = "Failed";
                }
